...
export type RequestInput = { id?: string | undefined; collectionId: string; name: string; topic: string; payloadTemplate: string; payloadFormat?: "raw" | "xml" | "json" | undefined; qos?: number | undefined; retain?: boolean | number | undefined; brokerProfileId?: string | null | undefined };
...
export const schemas = {
  ...
  request: z.object({ id: z.string().optional(), collectionId: z.string().min(1), name: z.string().min(1), topic: z.string().default(""), payloadTemplate: z.string().default("{}"), payloadFormat: z.enum(["raw", "xml", "json"]).default("json"), qos: z.number().int().min(0).max(2).default(0), retain: z.boolean().default(false), brokerProfileId: z.string().optional().nullable() }),
  variableCollection: z.object({ id: z.string().optional(), name: z.string().trim().min(1) }),
  variable: z.object({ id: z.string().optional(), variableCollectionId: z.string().min(1), name: z.string().trim().regex(/^[A-Za-z0-9_]+$/, "Variable name may only contain letters, numbers, and underscores."), value: z.string().default(""), sortOrder: z.number().int().nonnegative().optional() }),
